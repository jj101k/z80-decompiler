...
import { UnknownClampedValue, UnknownMemoryValue, UnknownValue } from "../Value/index.mjs"
...
export class MachineStateDumper {
     ...
     * @param {Set<import("../Z80Registers.mjs").Z80AnyRegister>} skipRegister
     * @param {Record<number, import("../UtilityTypes.mjs").anyValue | null>} activeMemoryValues
     * @param {Record<string, {location:
     * import("../UtilityTypes.mjs").anyValue,length: number, value:
     * import("../UtilityTypes.mjs").anyValue | null}>} activeMemoryExpressions
     * @param {Partial<Record<import("../Z80Registers.mjs").Z80AnyRegister, import("../UtilityTypes.mjs").anyValue | null>>} activeRegisterValues
     * @returns
     */
    #$dumpStates(skipMemory, skipRegister, activeMemoryValues, activeMemoryExpressions, activeRegisterValues) {
        if(Object.keys(activeMemoryValues).length + Object.keys(activeRegisterValues).length + Object.keys(activeMemoryExpressions).length == 0) {
            return []
        }
        ...
        const memoryExpresionValues = Object.values(activeMemoryExpressions).map((v) => `(${this.#$word(v.location)})=${this.$memoryValue([v.value])}`)
        /**
         * @type {Partial<Record<import("../Z80Registers.mjs").Z80AnyRegister, string>>}
         */
        ...
        return [...memoryValues, ...memoryExpresionValues, ...registerValues]
    }
     ...
     */
    #$word(v) {
        if(v instanceof UnknownClampedValue && v.bits >= 16) {
            return this.#$word(v.value)
        }
        if(Utilities.isNumber(v)) {
            return Formatter.$(v)
        } else if(v) {
     ...
     * @returns
     */
    $dump(skipMemory, skipRegister, activeMemoryValues, activeMemoryExpressions, activeRegisterValues, reset) {
        /**
         * @type {string[]}
         */
        const stateInfo = []
        ...
        stateInfo.push(...this.#$dumpStates(skipMemory, skipRegister, activeMemoryValues, activeMemoryExpressions, activeRegisterValues))
    ...
    $memoryValue(vs) {
        if(vs.length == 1) {
            ...
            return Formatter.$(this.#abbreviateMiddle(vs.map(v => {
                if(Utilities.isNumber(v)) {
