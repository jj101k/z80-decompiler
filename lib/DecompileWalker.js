...
class DecompileWalker extends CodeDecompiler {
    ...
    /**
     * @type {Record<number, {rel: number[], direct: number[]}}
     */
    #jumps = {}
    ...
    *dump() {
        ...
        for(const startPoint of startPoints) {
            ...
            if(startPoint > offset + 1) {
                yield `${this.addr(offset + this.loadPoint)}: (?)`.padEnd(40, " ") + `## (${startPoint-offset} bytes...)`
                offset = startPoint
            }
            const [n, l] = this.#seen.get(startPoint)
            const bytes = this.u8(...this.dw.inspectAt(startPoint, l))
            yield `${this.addr(startPoint + this.loadPoint)}: ${n}`.padEnd(40, " ") + `## ${bytes}`
            offset = startPoint + l
        }
    }
