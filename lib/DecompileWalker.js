...
class DecompileWalker extends CodeDecompiler {
    ...
    /**
     * @type {Record<number, {rel: number[], direct: number[], id: number}}
     */
    #jumps = {}
    ...
    /**
     * @type {Map<number, [DecomposedInstruction, number]>}
     */
    #seen = new Map()
    ...
    #noteJump(n) {
        if(!this.#jumps[n]) {
            this.#jumps[n] = {rel: [], direct: [], id: Object.keys(this.#jumps).length}
        }
    ...
    #noteRelJump(n) {
        ...
        if(!this.#jumps[address]) {
            this.#jumps[address] = {rel: [], direct: [], id: Object.keys(this.#jumps).length}
        }
    ...
    get simpleOpcodes() {
        return {
            ...
            [0xcb]: new CB(),
            [0xcd]: new CondJumpInstructionHandler("CALL a", this),
    ...
    decode() {
        ...
        try {
            ...
            this.#seen.set(this.#startPoint, [new DecomposedInstruction("(bad)"), this.dw.offset - this.#startPoint])
     ...
     */
    *dump() {
        const relOnly = Object.entries(this.#jumps).filter(([n, j]) => j.direct.length == 0)
        const directOnly = Object.entries(this.#jumps).filter(([n, j]) => j.rel.length == 0)
        const mixed = Object.entries(this.#jumps).filter(([n, j]) => j.rel.length && j.direct.length)

        yield "--- known jump targets ---"
        yield `Relative = ${relOnly.length}`
        const directOnlyTop = directOnly.slice().sort(
            ([na, ja], [nb, jb]) => jb.direct.length - ja.direct.length).slice(0, 10).sort(
                ([na], [nb]) => +na - +nb)
        yield `Direct = ${directOnly.length}`
        yield "Top direct are:"
        yield directOnlyTop.map(([n, j]) => `${j.direct.length} -> ${this.addr(+n)}`).join("\n")
        yield "Mixed relative/direct are:"
        yield mixed.map(([n, j]) => `* -> ${this.addr(+n)}`).join("\n")
        yield "--- dumping code ---"

        const labels = Object.fromEntries(Object.entries(this.#jumps).map(([n, j]) => [n, `j${j.id.toString(16)}`]))
    ...
    semanticDecode(n) {
        ...
        switch(n.pre) {
            ...
            case 0b11: { // 0xc-0xf
                ...
                if(jcrR[n.b3]) {
                    if(jcrR[n.b3] == "RET") {
                        return new DecomposedInstruction(`${jcrR[n.b3]} ${fR[n.a3]}`)
                    } else {
                        const a = this.dw.uint16()
                        this.addTarget(a)
                        return new DecomposedInstruction(`${jcrR[n.b3]} ${fR[n.a3]} a`, a)
                    }
