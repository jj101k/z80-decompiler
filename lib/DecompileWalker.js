...
class DecompileWalker extends CodeDecompiler {
    ...
    *dump() {
        ...
        for(const startPoint of startPoints) {
            ...
            yield `${(label ?? "").padEnd(7, " ")} ${caption}`.padEnd(40, " ") +
                `; ${bytes.padEnd(11, " ")} @${this.u16r(startPoint + this.loadPoint)}`
    ...
    semanticDecode(n) {
        ...
        switch(n.pre) {
            case 0b00: { // 0x0-0x3
                ...
                if(n.b4 == 0b0001) {
                    return new DecomposedInstruction(`LD ${rpR[n.a2]}, nn`, this.dw)
                    ...
                    return new DecomposedInstruction(`LD ${d}, n`, this.dw)
                }
                break
            }
            case 0b01: { // 0x4-0x7
            ...
            case 0b11: { // 0xc-0xf
                ...
                if(n.b3 == 0b000) {
                    ...
                    const di = new DecomposedInstruction(`JP ${fR[n.a3]} a`, this.dw)
                    this.addTarget(di.arguments[0])
                    return di
                } else if(n.b3 == 0b100) {
                    const di = new DecomposedInstruction(`CALL ${fR[n.a3]} a`, this.dw)
                    this.addTarget(di.arguments[0], "fn")
                    return di
                } else if(n.b3 == hlIndirect) {
                    // 8-bit arithmetic & logic
                    const op = arOpR[n.a3]
                    return new DecomposedInstruction(`${op} n`, this.dw)
                }
                break
            }
        }
