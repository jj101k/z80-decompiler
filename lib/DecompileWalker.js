            ...
            if(startPoint > offset) {
                ...
                for(const region of regions) {
                    if(region.start - this.loadPoint > offset) {
                        yield out(`DS ${region.start - this.loadPoint - offset}`, "...", offset)
                    }
                    seenMemoryRegions.add(region.start)
                    yield out(`DS ${region.length}`, "...", offset, labels[offset + this.loadPoint] ?? "")
                    offset = region.start - this.loadPoint + region.length
                }
                if(startPoint > offset) {
                    yield out(`DS ${startPoint - offset}`, "...", offset)
                }
                offset = startPoint
            }
