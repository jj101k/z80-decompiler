...
class DecompileWalker extends CodeDecompiler {
    ...
    *dump() {
         ...
         */
        const memoryRegions = []
        const orderedMemoryLocations = [...this.#memoryLocations, ...this.#probableMemoryLocations].sort((a, b) => a - b)
        if(orderedMemoryLocations.length) {
            let currentMemoryRegion = {start: orderedMemoryLocations[0], length: 0}
            memoryRegions.push(currentMemoryRegion)
            for(const l of orderedMemoryLocations) {
        ...
        const memoryRegionsInRange = memoryRegions.filter((r) => r.start >= this.loadPoint && r.start < this.dw.length + this.loadPoint)
        ...
        const ml = (memoryRegionsInRange.length - 1).toString().length
        ...
        const labels = Object.fromEntries([
            ...
            ...memoryRegionsInRange.map((r, i) => [r.start, `mr${i.toString().padStart(ml, "0")}`]),
         ...
         * @type {Set<number>}
         */
        const seenMemoryRegions = new Set()
        ...
        for(const startPoint of startPoints) {
            ...
            if(startPoint > offset) {
                const regions = memoryRegionsInRange.filter(r => r.start - this.loadPoint >= offset && r.start - this.loadPoint < startPoint)
                for(let i = 0; i < regions.length; i++) {
                    const region = regions[i]
                    if(region.start - this.loadPoint > offset) {
                        const l = region.start - this.loadPoint - offset
                        yield out(`DS ${l}`, l, offset)
                        offset = region.start - this.loadPoint
                    }
                    seenMemoryRegions.add(region.start)
                    const expectedRegionEnd = region.start - this.loadPoint + region.length
                    /**
                     * @type {(typeof regions[0]) | undefined}
                     */
                    const nextRegion = regions[i + 1]
                    const nextPoint = nextRegion ? nextRegion.start - this.loadPoint : startPoint
                    // Auto-expand
                    if(expectedRegionEnd < nextPoint) {
                        const l = nextPoint - offset
                        yield out(`DS ${l}`, l, offset, labels[offset + this.loadPoint] ?? "")
                        offset = nextPoint
                    } else {
                        yield out(`DS ${region.length}`, region.length, offset, labels[offset + this.loadPoint] ?? "")
                        offset = expectedRegionEnd
                    }
                }
                if(startPoint > offset) {
        ...
        const missingMemoryRegions = memoryRegionsInRange.filter(r => !seenMemoryRegions.has(r.start))
        if(missingMemoryRegions.length) {
            console.warn("Missing memory regions", missingMemoryRegions.map(r => `${this.addr(r.start)}(${r.length})`))
        }
    }
}
