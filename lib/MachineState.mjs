...
export class MachineState {
    ...
    #storeAtomicRegisterValueExpanded(register, n) {
        this.#activeRegisterValues[register] = AnyExpression.as8bit(n)
    ...
    dump(labels = {}) {
        ...
        const byteRun = (vs) => {
            if(vs.length == 1) {
                ...
                return "$" + abbreviateMiddle(vs.map(v => {
                    if(Utilities.isNumber(v)) {
    ...
    getWideRegisterValue(register) {
        ...
        for(const [i, sr] of srs.entries()) {
            ...
            vOut = AnyExpression.add(vOut, AnyExpression.shiftLeft(v, (srs.length - i - 1) * 8))
        }
        return vOut
    }
    ...
    storeWideRegisterValue(register, n) {
        ...
        if(n === null) {
            for(const sr of srs) {
                this.#storeAtomicRegisterValueExpanded(sr, null)
            }
        } else {
            ...
            if(srs.length == 1) {
                ...
                // } else {
                //     // for(const [i, sr] of srs.entries()) {
                //     //     this.#storeAtomicRegisterValueExpanded(sr, n >> ((srs.length - i - 1) * 8))
                //     // }
                //     for(const [i, sr] of srs.entries()) {
                //         this.#storeAtomicRegisterValueExpanded(sr, AnyExpression.shiftRight(n, (srs.length - i - 1) * 8))
                //     }
                // }
                for(const [i, sr] of srs.entries()) {
                    this.#storeAtomicRegisterValueExpanded(sr, AnyExpression.shiftRight(n, (srs.length - i - 1) * 8))
