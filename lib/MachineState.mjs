...
export class MachineState {
    ...
    #getDumpExcludeList(lastStates) {
        ...
        if (lastStates?.length) {
            ...
            for(const r of Object.values(Z80Registers8b)) {
                const v = this.#activeRegisterValues[r]
                if(v === undefined) {
                    continue
                }
                const expected = v?.toString()
    ...
    dump(labels = {}, lastStates = undefined) {
        ...
        const { skipMemory, skipRegister } = this.#getDumpExcludeList(lastStates)
        return dumper.$dump(skipMemory, skipRegister, this.#activeMemoryValues, this.#activeRegisterValues)
    }
