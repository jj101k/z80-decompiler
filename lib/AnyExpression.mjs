...
export class AnyExpression {
    ...
    static #shiftRightNonNull(a, b) {
        ...
        if(a instanceof UnknownExpressionValue && a.op == "+") {
            ...
            const fullyBelowValues = [a.a, a.b].filter(v => this.isNbit(v, b))
            const fullyAboveValues = [a.a, a.b].filter(v => this.isNbitZero(v, b))
    ...
    static as8bit(v) {
        if(v === null) {
            ...
            if(v.value instanceof UnknownExpressionValue && v.value.op == "+") {
                const {a, b} = v.value
                const nonFullyAboveValues = [a, b].filter(v => !this.isNbitZero(v, 8))
        ...
        } else if(v instanceof UnknownExpressionValue && v.op == "&") {
            if(this.isNbit(v.a, 8) || this.isNbit(v.b, 8)) {
                return v
            }
        }
    ...
    /**
     *
     * @param {import("./UtilityTypes.mjs").anyValue} v
     * @param {number} n
     */
    static isNbit(v, n) {
        return (typeof v == "number" && v < (1 << n)) || (v instanceof EffectivelyClampedValue && v.bits <= n)
    }
    /**
     * True if the bottom n bits are (must be) 0.
     *
     * @param {import("./UtilityTypes.mjs").anyValue} v
     * @param {number} n
     * @returns {boolean}
     */
    static isNbitZero(v, n) {
        return (v instanceof UnknownExpressionValue && v.op == "*" && (this.isNbitZero(v.a, n) || this.isNbitZero(v.b, n))) ||
            (typeof v == "number" && v % (1 << n) == 0)
    }
    /**
     *
     * @param {import("./UtilityTypes.mjs").anyValue | null} a
