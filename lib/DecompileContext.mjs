...
export class DecompileContext extends CodeDecompiler {
    ...
    decode() {
        ...
        if(this.#jumps[this.dw.offset + this.loadPoint]?.type === "fn") {
    ...
    *dump() {
         ...
         * @returns
         */
        const out = (content, length, location, label = "", states = [], lastStates = undefined) => {
            return [
                label.padEnd(7, " "),
                content.padEnd(16, " "),
                ";",
                ...
                ";",
                states.map(s => s.dump(labels, lastStates)).join("; ")
            ].join(" ")
        }
        ...
        for(const startPoint of startPoints) {
            ...
            yield out(caption, l, startPoint, label ?? "", currentStates, lastStates)
