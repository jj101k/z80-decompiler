    ...
    decode(entryPoints, memoryLocations) {
        ...
        for (i = 0; i < this.limit; i++) {
            ...
            if (!n) {
                ...
                throw new Error(`Cannot decode value at ${this.#filename}+${startPoint} (${decompile.addr(startPoint + this.loadPoint)}, from ${this.#filename}@${decompile.addr(decompile.entryPoint)}) after ${i} points (${bytesParsed} bytes) mapped: ${decompile.u8(...dw.inspect())}`)
            }
