...
export class DecompileWalker extends CodeDecompiler {
    ...
    get outOfRangeContent() {
        ...
        const functions = Object.entries(this.#jumps).filter(([n, j]) => j.type == "fn").map(([n]) => +n)
        const jumps = Object.entries(this.#jumps).filter(([n, j]) => j.type == "j").map(([n]) => +n)
     ...
     */
    addJumpToHL() {
        if(this.#activeHLValue !== null) {
            this.#jumpTo = this.addTarget(this.#activeHLValue)
        } else {
