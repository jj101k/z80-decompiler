...
export class XDInstructionHandler extends IncompleteInstructionHandler {
    ...
    get simpleOpcodes() {
        return {
            ...
            [0x23]: new AdHocParsingInstructionHandler(`INC ${this.offsetRegister}`, (dw, context) => {
                const v = context.s16.getRegisterValue(this.offsetRegister)
                context.s16.storeRegisterValue(this.offsetRegister, AnyExpression.add(v, 1))
            }),
            [0x2a]: new LoadInstructionHandler(`LD ${this.offsetRegister}, (nn)`, this.offsetRegister),
            [0x36]: new AdHocParsingInstructionHandler(`LD (${this.offsetRegister}+d), n`, (dw, context, di) => {
                context.s8.storeIndirectMemoryValue(this.offsetRegister, +di.arguments[1], +di.arguments[0])
            }),
            [0xb0]: new AdHocParsingInstructionHandler("OR B", (dw, context) => {
            ...
            [0xe5]: new AdHocParsingInstructionHandler(`PUSH ${this.offsetRegister}`, () => {
                // No-op for now
            }),
            [0xe3]: new AdHocParsingInstructionHandler(`EX (SP), ${this.offsetRegister}`, (dw, context) => {
                const v = context.s16.getRegisterValue(this.offsetRegister)
        ...
        switch (n.pre) {
            case 0b00: { // 0x0-3
                ...
                if (this.isToMemory(n) && (n.b3 & 0b110) == 0b100) { // 0x *d 34/ 0x *d 35
                    ...
                    return new AdHocParsingInstructionHandler(`${op} (${this.offsetRegister}+d)`, (dw, context, di) => {
                        context.s8.updateIndirectMemoryValue(this.offsetRegister,
                            v => (op == "INC") ? AnyExpression.add(v, 1) : AnyExpression.subtract(v, 1), +di.arguments[0])
                    })
                }
            ...
            case 0b01: { // 0x4-7
                if ((register = this.sourceRegister(n))) {
                    const r = register
                    return new AdHocParsingInstructionHandler(`LD (${this.offsetRegister}+d), ${r}`, (dw, context, di) => {
            ...
            case 0b10: { // 0x8-b
                if (this.isFromMemory(n)) { // 0x?e, 0x?6
                    const op = arOpR[n.a3]
                    return new AdHocParsingInstructionHandler(`${op.name} (${this.offsetRegister}+d)`, (dw, context, di) => {
