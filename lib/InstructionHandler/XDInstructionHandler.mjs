...
export class XDInstructionHandler extends IncompleteInstructionHandler {
    ...
    get simpleOpcodes() {
        return {
            ...
            [0x23]: new ParsingInstructionHandler(`INC ${this.offsetRegister}`, (dw, context) => {
            ...
            [0x36]: new ParsingInstructionHandler(`LD (${this.offsetRegister}+d), n`),
            [0xb0]: new ParsingInstructionHandler("OR B"),
            ...
            [0xe5]: new ParsingInstructionHandler(`PUSH ${this.offsetRegister}`),
            [0xe3]: new ParsingInstructionHandler(`EX (SP), ${this.offsetRegister}`),
        }
    }
        ...
        switch (n.pre) {
            case 0b00: { // 0x0-3
                ...
                if (this.isToMemory(n) && (n.b3 & 0b110) == 0b100) { // 0x *d 34/ 0x *d 35
                    ...
                    return new ParsingInstructionHandler(`${op} (${this.offsetRegister}+d)`)
                }
            ...
            case 0b01: { // 0x4-7
                if ((register = this.sourceRegister(n))) {
                    return new ParsingInstructionHandler(`LD (${this.offsetRegister}+d), ${register}`)
            ...
            case 0b10: { // 0x8-b
                if (this.isFromMemory(n)) { // 0x?e, 0x?6
                    const op = arOpR[n.a3]
                    return new ParsingInstructionHandler(`${op.name} (${this.offsetRegister}+d)`, (dw, context, di) => {
