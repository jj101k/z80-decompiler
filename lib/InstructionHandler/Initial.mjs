...
export class Initial extends InstructionHandler {
    ...
    get simpleOpcodes() {
        return {
            ...
            [0x0a]: new IndirectLoadInstructionHandler(Z80Registers.A, Z80Registers.BC),
            ...
            [0x1a]: new IndirectLoadInstructionHandler(Z80Registers.A, Z80Registers.DE),
            [0x1f]: new TrivialInstructionHandler("RRA"),
            [0x20]: new RelCondJumpInstructionHandler("JR NZ, e"),
            [0x21]: new UpdateRegisterInstructionHandler("LD HL, nn", Z80Registers.HL),
            [0x22]: new SaveInstructionHandler("LD (nn), HL"),
            ...
            [0x31]: new UpdateRegisterInstructionHandler("LD SP, nn", Z80Registers.SP),
            [0x32]: new SaveInstructionHandler("LD (nn), A"),
            ...
            [0x3a]: new LoadInstructionHandler("LD A, (nn)", Z80Registers.A),
            ...
            [0xdd]: new DD(),
            [0xe1]: new ImportRegisterInstructionHandler("POP HL", Z80Registers.HL),
            ...
            [0xf9]: new CopyRegisterInstructionHandler(Z80Registers.SP, Z80Registers.HL),
    ...
    get(dw, context) {
        ...
        switch(n.pre) {
            case 0b00: { // 0x0-0x3
                const r = addHlIxIy(n, Z80Registers.HL)
                if(r) {
                    return r
                }
            ...
            case 0b01: { // 0x4-0x7
                if((register = this.targetRegister(n))) {
                    const activeHLValue = context.getRegisterValue(Z80Registers.HL)
                    if(activeHLValue) {
                        context.addMemoryLocation(activeHLValue)
                    }
                    return new IndirectLoadInstructionHandler(register, Z80Registers.HL).resolve(dw, context)
            ...
            case 0b10: { // 0x8-0xb
                ...
                context.storeRegisterValue(Z80Registers.A, context.getOpResult(op, context.getRegisterValue(Z80Registers.A), context.getValue(r)))
            ...
            case 0b11: { // 0xc-0xf
                ...
                switch(n.b3) {
                    ...
                    case 0b110: {
                        ...
                        const op = arOpR[n.a3]
                        const di = new DecomposedInstruction(`${op.name} n`, dw)
                        context.storeRegisterValue(Z80Registers.A, context.getOpResult(op, context.getRegisterValue(Z80Registers.A), +di.arguments[0]))
                        return di
                    }
                }
                break
