...
export class IndirectLoadInstructionHandler extends TrivialInstructionHandler {
    ...
    get(dw, context) {
        ...
        let v = null
        if(this.#withOffset) {
            v = di ? context.state.getIndirectMemoryValue(this.#indirectRegister, 1, +di?.arguments[0]) : null
        } else {
            v = context.state.getIndirectMemoryValue(this.#indirectRegister)
        }
        context.state.storeRegisterValue(this.#register, v)
        return di
    }
