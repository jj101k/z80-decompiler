...
export class XD extends InstructionHandler {
    ...
    get simpleOpcodes() {
        return {
            ...
            [0x22]: new SaveInstructionHandler(`LD (nn), ${this.offsetRegister}`, this.offsetRegister, 2),
            [0x23]: new TrivialInstructionHandler(`INC ${this.offsetRegister}`).withAction((dw, context) => {
                const v = context.state.getRegisterValue(this.offsetRegister)
                if(v !== null) {
                    context.state.storeRegisterValue(this.offsetRegister, v + 1)
                }
            }),
            [0x2a]: new LoadInstructionHandler(`LD ${this.offsetRegister}, (nn)`, this.offsetRegister, 2),
        ...
        switch (nnx.pre) {
            ...
            case 2: { // 0x8-b
                if (this.isFromMemory(nnx)) { // 0x?e, 0x?6
                    ...
                    context.state.updateRegisterValue(Z80Registers8B.A, op, context.state.getIndirectMemoryValue(
                        this.offsetRegister, 1, +di.arguments[0]))
                    return di
                }
                break
            }
