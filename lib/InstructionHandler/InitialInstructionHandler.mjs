...
export class InitialInstructionHandler extends IncompleteInstructionHandler {
    ...
    get simpleOpcodes() {
        return {
            [0x00]: new ParsingInstructionHandler("NOP"),
            [0x02]: new ParsingInstructionHandler("LD (BC), A"),
            [0x07]: new ParsingInstructionHandler("RLCA"),
            [0x08]: new ParsingInstructionHandler("EX AF, AF'"),
            [0x0a]: new IndirectLoadInstructionHandler(Z80Registers8b.A, Z80Registers16b.BC),
            [0x0f]: new ParsingInstructionHandler("RRCA"),
            ...
            [0x36]: new ParsingInstructionHandler("LD (HL), n", (dw, context, di) => {
            ...
            [0x37]: new ParsingInstructionHandler("SCF"),
            [0x38]: new RelCondJumpInstructionHandler("JR C, e"),
            [0x3a]: new LoadInstructionHandler("LD A, (nn)", Z80Registers8b.A),
            [0x3f]: new ParsingInstructionHandler("CCF"),
            [0xc3]: new UncondJumpInstructionHandler("JP a"),
            [0xc9]: new ParsingInstructionHandler("RET", (dw, context) => {
                context.unknownJump = true
            }),
            [0xcb]: new CBInstructionHandler(),
            [0xcd]: new CallInstructionHandler("CALL a"),
            [0xd3]: new ParsingInstructionHandler("OUT (n), A"),
            [0xd9]: new ParsingInstructionHandler("EXX"),
            [0xdb]: new ParsingInstructionHandler("IN A, (n)"),
            [0xdd]: new DDInstructionHandler(),
            [0xe1]: new ImportRegisterInstructionHandler("POP HL", Z80Registers16b.HL),
            [0xe3]: new ParsingInstructionHandler("EX (SP), HL"),
            [0xe9]: new ParsingInstructionHandler("JP (HL)", (dw, context) => {
                context.addJumpToHL()
            }),
            [0xeb]: new ParsingInstructionHandler("EX DE, HL"),
            [0xed]: new EDInstructionHandler(),
            [0xf3]: new ParsingInstructionHandler("DI"),
            [0xf9]: new CopyRegisterInstructionHandler(Z80Registers16b.SP, Z80Registers16b.HL),
            [0xfb]: new ParsingInstructionHandler("EI"),
        ...
        switch(n.pre) {
            case 0b00: { // 0x0-0x3
                ...
                if(n.b4 == 0b0001) { // 0x01 11 [21] 31
                    ...
                    if(r == "(HL)") {
                        return new ParsingInstructionHandler(`${op} ${r}`, (dw, context) => {
                            const address = context.s16.getRegisterValue(Z80Registers16b.HL)
                            if(Utilities.isNumber(address)) {
                                const v = context.s8.getMemoryValue(address)
                                const vm = (op === "INC") ? AnyExpression.add(v, 1) : AnyExpression.subtract(v, 1)
                                context.s8.storeMemoryValue(address, vm)
                            } else {
                    ...
                    switch(n.b4) {
                        case 0b0011: { // INC
                            return new ParsingInstructionHandler(`INC ${rp}`, (dw, context) => {
                                const v = context.s16.getRegisterValue(rp)
                                context.s16.storeRegisterValue(rp, AnyExpression.add(v, 1))
                            })
                        }
                        case 0b1011: { // DEC
                            return new ParsingInstructionHandler(`DEC ${rp}`, (dw, context) => {
                                const v = context.s16.getRegisterValue(rp)
            ...
            case 0b01: { // 0x4-0x7
                if((register = this.targetRegister(n))) {
                    ...
                    const r = register
                    return new ParsingInstructionHandler(`LD (HL), ${r}`, (dw, context) => {
                        const activeHLValue = context.s16.getRegisterValue(Z80Registers16b.HL)
                        if(Utilities.isNumber(activeHLValue) && activeHLValue) {
                            context.addMemoryLocation(activeHLValue)
                            const v = context.s8.getRegisterValue(r)
                            context.s8.storeMemoryValue(activeHLValue, v)
                        }
                    })
                } else {
            ...
            case 0b10: { // 0x8-0xb
                ...
                if(r == "(HL)") {
                    return new ParsingInstructionHandler(`${op.name} ${r}`, (dw, context) => {
            ...
            case 0b11: { // 0xc-0xf
                if(n.b4 == 0b0001 && n.a2 != hlR) { // 0xc1, d1, [e1], f1
                    ...
                    return new ParsingInstructionHandler(`PUSH ${rpR[n.a2]}`)
                } else if(n.b3 == 0b111) { // 0x[c-f]f; 0[c-f]7
                    return new ParsingInstructionHandler(`RST ${n.a3 << 3}h`)
                }
                const fR = {
                    [0b000]: "NZ",
                    [0b001]: "Z",
                ...
                switch(n.b3) {
                    case 0b000: {
                        return new ParsingInstructionHandler(`RET ${fR[n.a3]}`)
                    }
                    case 0b010: {
                        return new ParsingInstructionHandler(`JP ${fR[n.a3]} a`, (dw, context, di) => {
                            context.addTarget(di.arguments[0])
                        })
                    }
                    case 0b100: {
                        return new ParsingInstructionHandler(`CALL ${fR[n.a3]} a`, (dw, context, di) => {
                    ...
                    case 0b110: {
                        ...
                        const op = arOpR[n.a3]
                        return new ParsingInstructionHandler(`${op.name} n`, (dw, context, di) => {
