...
describe("Expression tests", () => {
    ...
    it("does not clamp ([clamped] & [unclamped])", () => {
        const ms = new MachineState()
        const hl = ms.getWideRegisterValue(Z80Registers16b.HL)
        expect(AnyExpression.as8bit(AnyExpression.and(hl, 5))).toBeInstanceOf(UnknownExpressionValue)
        expect(AnyExpression.as8bit(AnyExpression.and(hl, ms.getNarrowRegisterValue(Z80Registers8b.A)))).toBeInstanceOf(UnknownExpressionValue)
    })
})